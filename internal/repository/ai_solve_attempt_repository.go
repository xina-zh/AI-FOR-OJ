...
func (r *GORMAISolveAttemptRepository) ListByRunID(ctx context.Context, runID uint) ([]model.AISolveAttempt, error) {
	...
	if err := r.listByRunIDQuery(ctx, runID).Find(&attempts).Error; err != nil {
		return nil, err
	}
	return attempts, nil
}

func (r *GORMAISolveAttemptRepository) listByRunIDQuery(ctx context.Context, runID uint) *gorm.DB {
